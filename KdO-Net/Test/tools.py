...
def get_basement_keypts(pcd, keyptspath, filename):
    pts = np.array(pcd.points)
    print('****************')
...
def get_desc(descpath, filename, desc_name):
    if desc_name == 'EdsNet':
        desc = np.load(os.path.join(descpath, filename + '.ply_0.150000_16_1.desc.EdsNet.bin.npy'))
    elif desc_name == 'KdO-Net':
        desc = np.load(os.path.join(descpath, filename + '.ply_0.150000_16_1.desc.KdO-Net.bin.npy'))
