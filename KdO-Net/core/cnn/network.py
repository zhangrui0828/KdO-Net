...
class NetworkBuilder(object):
    ...
    def _build_placeholder(self):
        ...
        self.anchor_input = tf.placeholder(dtype=tf.float32, shape=[None, int(np.cbrt(self.config.input_dim)),
                                           ...
                                           name='X_reference')
        print("进入network.py", self.anchor_input)  
        ...
        self.starter_learning_rate = self.config.learning_rate
    ...
    def _build_model(self):
        """Build KdO-Net network for testing."""

        # -------------------- Network archintecture --------------------
        from core.cnn.architecture import network_architecture

        # Build graph
        print("Building the KdO-Net graph")

        self.keep_probability = tf.placeholder(tf.float32)

        # Build network for training using the tf_records files
        self.anchor_output, self.positive_output, _, _ = network_architecture(self.anc_training_batch,
        ...
        self.test_anchor_output, self.test_positive_output, self.input_anc_list, self.input_pos_list = network_architecture(self.anchor_input,
                                                                                  self.positive_input,
                                                                                  self.keep_probability, self.config,
                                                                                  reuse=True)
    ...
    def _build_optim(self):
        ...
        learning_rate = tf.compat.v1.train.cosine_decay_restarts(self.starter_learning_rate, self.global_step,
        ...
        optimizer = tf.compat.v1.train.AdadeltaOptimizer(learning_rate).minimize(self.losses, global_step=self.global_step)
    ...
    def _build_writer(self):
        ...
        self.base_file_name = 'lr_{}_batchSize_{}_outDim_{}'.format(self.config.learning_rate,
    ...
    def train(self, num_epoch):
        ...
        if not os.path.exists(self.config.validation_data_folder):
            ...
            raise ValueError(
                'The validation data directory {} does not exist.'.format(self.config.validation_data_folder))
        ...
        if self.config.resume_flag:
            ...
            self.step = 0
            print("Starting from the global step {}.".format(self.step))
        ...
        for self.step in trange(self.step, self.config.max_steps, ncols=79):
            ...
            if (self.step + 1) % self.config.evaluate_rate == 0:  
                ...
                print('\nOnline training accuracy at epoch {} iterration {} equals {} percent!'.format(self.num_epoch, self.step,
            ...
            if (self.step + 1) % self.config.save_model_rate == 0:
                self.saver.save(self.sess,
                                save_path=self.config.saved_model_dir + '{}_dim/'.format(self.config.output_dim) +
                                           self.base_file_name + '_epoch_{}_Iteration_{}.ckpt'.format(self.num_epoch, self.step))
            ...
            if (self.step + 1) % self.config.save_accuracy_rate == 0:
                ...
                self.writer.add_summary(summary_training, self.step+self.num_epoch*self.config.max_steps)
                self.writer.add_summary(summary_validation, self.step+self.num_epoch*self.config.max_steps)
            ...
            self.writer.add_summary(summary, self.step+self.num_epoch*self.config.max_steps)
            self.step += 1
            self.step = 0
    ...
    def test(self):
        ...
        model_file_name = 'lr_1.0_batchSize_128_outDim_32_epoch_1_Iteration_39999.ckpt'
        ...
        for file in evaluation_files:
            ...
            evaluation_features = np.fromfile(file, dtype=np.float32)  
            ...
            np.save(self.config.evaluate_output_folder + '/{}_dim/'.format(self.config.output_dim) +
                                evaluate_file_name[:-11] +  f".desc.KdO-Net.bin", all_predictions.astype(np.float32))
            print('Wrote file {0}'.format(evaluate_file_name[:-11] + '.desc.KdO-Net.bin.npy'))
