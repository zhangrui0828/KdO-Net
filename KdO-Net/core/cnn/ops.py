    ...
    with tf.name_scope(layer_name):
        with tf.variable_scope('', reuse=reuse):
            biases = tf.get_variable(layer_name + '_b', shape=shape,
                                     dtype=tf.float32,
                                     initializer=bias_init)  # default initialier: glorot_uniform_initializer
    return biases
...
def batch_iter(data, batch_size, num_epochs, shuffle=True):   #batch_size=1000, num_epochs=1
    """
    Generates a batch iterator for a dataset.
    """
    data = np.array(data)  # 生成数组
    data_size = len(data)
    print("ops.batch_iter():data_size", data_size)
    num_batches_per_epoch = int((len(data) - 1) / batch_size) + 1
    print("ops.batch_iter():num_batches_per_epoch", num_batches_per_epoch)
    for epoch in range(num_epochs):
        # Shuffle the data at each epoch
        if shuffle:
            shuffle_indices = np.random.permutation(np.arange(data_size))  # 打乱顺序
        ...
        for batch_num in range(num_batches_per_epoch):
            ...
            print("ops.batch_iter():start_index,end_index", start_index,end_index)
