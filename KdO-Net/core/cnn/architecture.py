...
def network_architecture(x_anc, x_pos, dropout_rate, config, reuse=False):
    ...
    with tf.name_scope('3DIM_cnn') as scope:
        for layer in np.arange(0, len(channels) - 2):
            ...
            input_anc_list.append(input_anc)
...
def conv_block(input_anc, input_pos, channels, dropout_flag, dropout_rate, laxer_idx, stride_input=1, k_size=3,
    ...
    conv_output_anc = tf.add(
        ops.conv3d(input_anc, weights, stride=[stride_input, stride_input, stride_input], padding=padding_type), bias)
    conv_output_pos = tf.add(
        ops.conv3d(input_pos, weights, stride=[stride_input, stride_input, stride_input], padding=padding_type), bias)
