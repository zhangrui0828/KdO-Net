...
def batch_hard(dists, pids, margin, batch_precision_at_k=None):
    ...
    with tf.name_scope("batch_hard"):
        ...
        with tf.compat.v1.Session() as sess:
            sess.run(init)
            a = furthest_positive.eval()
            b = closest_negative.eval()
            print("max_furthest_positive:", max(a))  
            print("min_furthest_positive:", min(a))  
            print("max_closest_negative", max(b))    
            print("min_closest_negative", min(b))    


        # compute the loss of furthest_positive with sigmoid
        furthest_positive = tf.sigmoid(furthest_positive)  
        # compute the loss of closest_negative with cross Entropy
        closest_negative = tf.math.exp(-closest_negative)
        closest_negative = tf.math.log(1 + closest_negative)   

        with tf.compat.v1.Session() as sess:
            sess.run(init)
            fp = furthest_positive.eval()
            cn = closest_negative.eval()
            print("compute loss")
            print("furthest_positive:", fp)
            print("closest_negative:", cn)

        diff = (furthest_positive + closest_negative)/2
    ...
    if batch_precision_at_k is None:
        tf.compat.v1.summary.scalar('loss', tf.reduce_mean(diff))
